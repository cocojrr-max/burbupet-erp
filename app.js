...
async function viewPayslip(id){
  const {data:x,error}=await db.from('payroll_items').select('*,workers(full_name,document_number,position,payment_type,pension_system),payroll_periods(*)').eq('id',id).single();if(error)return toast(error.message,true);
