...
async function loadPage(){const titles={home:state.profile?.app_role==='admin'?'Panel de gestión':'Mi portal',workers:'Trabajadores',attendance:'Asistencia',reports:'Reportes',schedules:'Horarios',payroll:'Planilla',requests:'Solicitudes y permisos'};$('#title').textContent=titles[state.page];$('#view').innerHTML='<div class="card empty">Cargando información…</div>';try{if(state.page==='home')await home();if(state.page==='workers')await workers();if(state.page==='attendance')await attendance();if(state.page==='reports')await reports();if(state.page==='schedules')await schedules();if(state.page==='payroll')await payroll();if(state.page==='requests')await requests()}catch(e){$('#view').innerHTML=`<div class="card empty">No se pudo cargar: ${esc(e.message)}</div>`}}
...
async function requestAttendanceExplanation(id,type='marcación'){const suggested=type==='permanencia'?'Por favor, sustenta el motivo por el que permaneciste después de tu hora de salida.':type==='falta'?'Por favor, justifica por qué no registraste ninguna marcación en esta fecha.':'Por favor, explica por qué falta una marcación en esta fecha.',message=prompt('Mensaje para el trabajador:',suggested);if(!message)return;const {error}=await db.from('attendance').update({review_status:'pending',admin_note:`Solicitud de ${type} del administrador: ${message}`}).eq('id',id);toast(error?error.message:'Solicitud enviada al portal del trabajador.',!!error);attendance()}
