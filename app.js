...
home=async function(){await homeUnfiltered();const current=todayLocal(),monthStart=current.slice(0,7)+'-01',admin=state.profile.app_role==='admin';let aq=db.from('attendance').select('id').eq('review_status','pending').gte('work_date',monthStart).lte('work_date',current),rq=db.from('requests').select('id').eq('status','pending');if(!admin){aq=aq.eq('worker_id',state.profile.worker_id);rq=rq.eq('worker_id',state.profile.worker_id)}const [{data:incidents},{data:requestsPending}]=await Promise.all([aq,rq]),count=(incidents||[]).length,pending=(requestsPending||[]).length;if(admin){const banner=$('#view .incident-banner span'),card=$('#view .dashboard-stats .accent-yellow strong');if(banner)banner.textContent=`${count} incidencias · ${pending} solicitudes`;if(card)card.textContent=count}else{const card=$('#view .accent-yellow strong');if(card)card.textContent=count}};
