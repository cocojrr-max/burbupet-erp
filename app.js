...
async function closePayrollPeriod(){const open=(state.payrollItems||[]).map(x=>x.payroll_periods).filter(x=>x?.status==='preparing').sort((a,b)=>b.end_date.localeCompare(a.end_date))[0];if(!open)return toast('No hay una planilla en preparación.',true);if(!confirm(`¿Cerrar la planilla del ${date(open.start_date)} al ${date(open.end_date)}? Después quedará lista para emitir boletas.`))return;const {error}=await db.from('payroll_periods').update({status:'closed'}).eq('id',open.id);toast(error?error.message:'Planilla cerrada y boletas listas.',!!error);if(!error)payroll()}
